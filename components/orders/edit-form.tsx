...
import { updateOrder, OrderState } from '@/lib/actions';
import {CustomerField, ProductsField, ProductLine, OrdersForm } from '@/lib/definitions';
...
const [currentLine, setCurrentLine] = useState<ProductLine>({
 ...
 useEffect(() => {
  ...
  setOrderData(prev => ({ ...prev, total: newTotal }));
  }, [orderLines, products]);
...
function addProductLine() {
  ...
  {orderLines.length === 0 ? (
    ...
    <p className="text-500 text-sm mt-2 text-right mr-5">
      Total: {USDollar.format(orderData.total)}
    </p>
  )}
      </div>
